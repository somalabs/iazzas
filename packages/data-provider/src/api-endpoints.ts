...
export const verifyAgent = (agent_id: string) =>
  `${agents({ path: `${encodeURIComponent(agent_id)}/verify` })}`;
...
export const bannerSeen = (bannerId: string) =>
  `${BASE_URL}/api/banners/${encodeURIComponent(bannerId)}/seen`;
