...
function KPICard({ label, value, subtitle }: CardProps) {
  return (
...
export default function KPICards({ kpis }: Props) {
  ...
  return (
    <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
      <KPICard label={localize('com_admin_analytics_total_tokens')} value={formatTokens(kpis.totalTokens)} />
      <KPICard
        label={localize('com_admin_analytics_credits_spent')}
        value={formatDisplayCredits(kpis.totalCreditsSpent)}
        subtitle={formatUSD(kpis.totalCreditsSpent)}
      />
      ...
      <KPICard
        ...
        value={formatDisplayCredits(kpis.avgCreditsPerUser)}
        subtitle={formatUSD(kpis.avgCreditsPerUser)}
      />
    </div>
  );
}
