...
const AutoRefillSettings: React.FC<AutoRefillSettingsProps> = ({
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-medium">{localize('com_ui_ux_balance_cycle')}</h3>
      <div className="mb-1 flex justify-between text-sm">
        <span>{localize('com_ui_ux_balance_ultimo_ciclo')}</span>
        <span>{lastRefillDate ? lastRefillDate.toLocaleString() : '-'}</span>
      </div>
      <div className="mb-1 flex justify-between text-sm">
        <span>{localize('com_ui_ux_balance_creditos_ciclo')}</span>
        <span>{refillAmount !== undefined ? refillAmount : '-'}</span>
      </div>
      <div className="mb-1 flex justify-between text-sm">
        <span>{localize('com_ui_ux_balance_frequencia')}</span>
        <span>
          {localize('com_ui_ux_balance_a_cada')} {refillIntervalValue}{' '}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <Label className="font-light">{localize('com_ui_ux_balance_proxima_virada')}</Label>
