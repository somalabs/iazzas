...
export default function RecentWork() {
  ...
  if (recent.length === 0) {
    return (
      <section className="mx-auto mt-12 w-full max-w-[760px] px-2">
    ...
    <section className="mx-auto mt-12 w-full max-w-[760px] px-2">
      ...
      <div className="grid grid-cols-2 gap-8 sm:grid-cols-3">
        {recent.map((c, i) => (
          ...
          >
            <div className="relative aspect-[4/5] overflow-hidden rounded-[14px] border border-rule bg-canvas">
              <img
              ...
              <span className="absolute bottom-2 right-3 text-[11px] text-on-action opacity-0 transition-opacity duration-150 group-hover:opacity-100">
                {localize('com_ui_open')}
              </span>
            </div>
            <p className="mt-2 line-clamp-1 font-editorial text-[16px] text-text-primary">
              {c.title || localize('com_ui_new_chat')}
            </p>
            <p className="text-xs text-text-tertiary">{formatDate(c.updatedAt)}</p>
          </button>
        ))}
      </div>
    </section>
