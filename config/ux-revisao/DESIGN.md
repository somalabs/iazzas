...
> **OVERRIDE AUTORITATIVO — Artur, 2026-05-18 (LEM-58, supersede LEM-53/54 só neste ponto):**
> A cadência da virada é **DIÁRIA**, não mensal. O display NÃO mostra mais "Vira
> DD/MM"; mostra **renovação diária às 00:00 America/Sao_Paulo**, ex.: **"Renova
> hoje 00:00 (em Xh)"** e o card vira **"Uso de hoje"**. A barra saldo/teto e os
> 3 estados de cor (verde / âmbar >70% / vermelho >90%) **permanecem inalterados**
> — só a cadência/microcópia muda. As seções abaixo que dizem "Vira DD/MM",
> "viraDDMM", `com_ui_ux_balance_vira_em` e o cálculo via `lastRefill`/
> `refillInterval*` ficam **superseded por esta nota**. Implementação (LEM-58):
> `getCycleInfo` agora expõe `hoursUntilRenewal` (helper puro
> `hoursUntilSaoPauloMidnight`, contagem até a próxima meia-noite SP, independente
> dos campos de intervalo do backend); chaves novas `com_ui_ux_balance_renova` =
> "Renova hoje 00:00" e `com_ui_ux_balance_renova_em` = "em". O mecanismo/política
> de cota diária (autoRefill, debitar etc.) é a issue dedicada **LEM-57**, FORA do
> escopo da LEM-52 — esta revisão só consome/exibe.
